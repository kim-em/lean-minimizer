...
/-- Dependency-based split heuristic.

    This heuristic:
    1. Analyzes the file to find which constants each command references/defines
    2. Builds a dependency DAG
    3. Finds all commands reachable from what the invariant section actually uses
    4. Suggests removing unreachable commands first

    The split returned puts likely-removable (unreachable) commands first. -/
unsafe def dependencyHeuristic : SplitHeuristic := fun state candidates => do
  -- Only apply heuristic at the top level (full candidate set)
  -- For recursive calls, fall back to default halving
  if candidates.size < state.markerIdx then
    defaultSplitHeuristic state candidates
  else
    -- Run the frontend to get compilation steps
    let steps ← runFrontend state.input state.fileName

    -- Split into commands before and after (including) the marker
    let stepsBeforeMarker := steps.filter (·.idx < state.markerIdx)
    let invariantSteps := steps.filter (·.idx ≥ state.markerIdx)

    if stepsBeforeMarker.isEmpty then
      defaultSplitHeuristic state candidates
    else
      -- Build dependency map for commands before marker
      let analyses := analyzeSteps stepsBeforeMarker
      let deps := buildDependencyMap analyses

      -- Find which commands define constants that the invariant section needs
      let invariantDeps := findInvariantDependencies stepsBeforeMarker invariantSteps

      if invariantDeps.isEmpty then
        -- Invariant doesn't reference anything from before marker, fall back
        defaultSplitHeuristic state candidates
      else
        -- Compute transitive closure of dependencies
        let reachable := computeReachable deps invariantDeps

        -- Split: unreachable commands go first (try to remove them), reachable go second
        ...
        if state.verbose then
          IO.eprintln s!"  Dependency analysis: {reachable.size} commands reachable, {unreachable.size} likely removable"

        -- If we can't identify any likely removable commands, fall back to default
        if unreachable.isEmpty || reachableInCandidates.isEmpty then
          defaultSplitHeuristic state candidates
        else
          return (unreachable, reachableInCandidates)

/-- Main minimization function using dependency-guided heuristic -/
unsafe def minimize (input : String) (fileName : String) (marker : String) (verbose : Bool) :
    IO String :=
  minimizeWith dependencyHeuristic input fileName marker verbose
