...
unsafe def emptyScopeRemovalPass : Pass where
  ...
  run := fun ctx => do
    ...
    if ← testCompilesSubprocess allRemovedSource ctx.fileName ctx.crossWorkspace then
    ...
    for pair in pairsToTry do
      ...
      if ← testCompilesSubprocess testSource ctx.fileName ctx.crossWorkspace then
        if ctx.verbose then
