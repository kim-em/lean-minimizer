...
def attributeExpansionPass : Pass where
  ...
  run := fun ctx => do
    ...
    if h : cmdIdx < ctx.steps.size then
      ...
      -- Verify it compiles
      let (success, errorOutput) ← testCompilesSubprocessWithError newSource ctx.fileName ctx.crossWorkspace
      if success then
        if ctx.verbose then
        ...
        let (simpleSuccess, simpleError) ← testCompilesSubprocessWithError simpleSource ctx.fileName ctx.crossWorkspace
